...
test("날짜만 확인된 접수에는 시간 확인 알림을 만들고 선택 시각만 예약한다", () => {
  const dateOnlyRace = makeRace({ registrationOpenTimeConfirmed: false });
  const target = core.getAlertTarget(dateOnlyRace, NOW);
  assert.equal(target.type, "registration_time_check");
  assert.equal(target.upgradeTargetKey, "registration");
  assert.deepEqual(
    core.computeTimeCheckTimes(target.at, ["09:00", "12:00"], NOW).map((item) => item.slot),
    ["09:00", "12:00"]
  );
});
...
test("같은 시각의 종목도 안정 ID로 각각 유지하고 시간 미확인 종목에는 확인 알림을 만든다", () => {
  ...
  assert.deepEqual(core.getRegistrationTargets(race, NOW).map((target) => target.key), ["time-check:window:5k", "window:full", "window:10k"]);
});

test("시간 확인 알림은 공식 시각이 확인되면 정확 접수 알림으로 전환된다", () => {
  const dateOnly = makeRace({ registrationOpenTimeConfirmed: false });
  const timeCheckTarget = core.getAlertTarget(dateOnly, NOW);
  const slots = ["09:00", "12:00"];
  const stored = {
    [core.subscriptionStorageKey(dateOnly.id, timeCheckTarget)]: {
      enabled: true,
      raceId: dateOnly.id,
      targetType: timeCheckTarget.type,
      targetKey: timeCheckTarget.key,
      upgradeTargetKey: timeCheckTarget.upgradeTargetKey,
      targetAt: timeCheckTarget.at,
      targetLabel: timeCheckTarget.label,
      offsets: [],
      checkTimes: slots,
      scheduledAlerts: core.computeTimeCheckTimes(timeCheckTarget.at, slots, NOW)
    }
  };
  const exactAt = "2026-07-13T10:00:00+09:00";
  const confirmed = makeRace({ registrationOpenAt: exactAt, registrationOpenTimeConfirmed: true });
  const result = core.reconcileSubscriptions(stored, [confirmed], { now: NOW });
  const next = result.alerts[confirmed.id];
  assert.equal(next.targetType, "registration_open");
  assert.equal(next.targetKey, "registration");
  assert.equal(next.targetAt, exactAt);
  assert.deepEqual(next.offsets, [20, 10, 0]);
  assert.deepEqual(next.checkTimes, []);
});
