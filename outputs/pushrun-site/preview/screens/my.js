// 마이 화면은 프로필, 배지, 운동 이력 캘린더, 외부 연동 상태를 나눠 보여 줍니다.
...
function renderCalendar() {
  ...
  return html`<section class="screen-heading"><span class="eyebrow">MY RECORDS</span><h1>달린 날이<br />한눈에 보여요.</h1><p>코칭 완주·수동 기록·나중에 연결한 건강 데이터를 출처와 함께 구분합니다.</p></section><section class="card calendar-card"><div class="calendar-heading"><div><span class="eyebrow">2026 JULY</span><h2>7월 기록 캘린더</h2><p>날짜를 누르면 그날의 운동 출처와 시간을 확인해요.</p></div><span class="calendar-legend">러닝 · 걷기 · 휴식</span></div><div class="calendar-weekdays"><span>일</span><span>월</span><span>화</span><span>수</span><span>목</span><span>금</span><span>토</span></div><div class="calendar-grid">${blanks}${days}</div></section><section class="section"><div class="section-title"><h2>기록 상세</h2><button class="text-button" data-action="manual-record">직접 기록</button></div><div class="card stack">${calendarDays.filter((item) => item.kind !== 'rest').map((item) => `<div class="list-row"><span><b>7월 ${item.day}일 · ${item.label}</b><small>${item.meta}</small></span><span class="mini-tag">${item.kind === 'run' ? '러닝' : '걷기'}</span></div>`).join('')}</div></section><section class="section"><div class="section-title"><h2>기록 연동</h2><span class="mini-tag">연결 전</span></div><div class="connection-grid"><article class="connection-card"><strong>Samsung Health</strong><p>Health Connect를 통해 Android 기기 기록을 가져오는 구조입니다.</p><button class="secondary" data-action="health-connect">연동 준비</button></article><article class="connection-card disabled"><strong>Garmin Connect</strong><p>Garmin Health API 파트너 승인과 사용자 동의가 필요한 별도 연결입니다.</p><button class="secondary" data-action="garmin-connect">승인 필요</button></article><article class="connection-card disabled"><strong>Nike Run Club</strong><p>공식 사용자 데이터 연동 가능 여부를 별도로 확인해야 합니다.</p><button class="secondary" data-action="nike-connect">검토 중</button></article></div></section><p class="notice">현재는 내 기기의 코칭·수동 기록만 저장합니다. 다른 앱의 기록은 각 회사의 공식 승인, 사용자 동의, 개인정보 처리방침이 준비된 뒤에만 가져옵니다.</p>`;
}
